...
class XXE1:
    ...
    def _recalc_xor_stream(self) -> None:
        ...
        block = [
            (working[i] + self._state[i]) & 0xFFFFFFFF
            for i in range(16)
        ]
    ...
    def process(self, data: bytes) -> bytes:
        ...
        while offset < len(src):
            ...
            chunk = min(
                len(src) - offset,
                self.process_bytes_at_time - self._keystream_offset,
            )
            key_stream = self._keystream[
                self._keystream_offset:self._keystream_offset + chunk
            ]

            for i in range(chunk):
                output[offset + i] = src[offset + i] ^ key_stream[i]
